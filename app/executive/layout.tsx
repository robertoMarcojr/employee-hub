...
import { LayoutDashboard, Briefcase, BarChart3, Users, LogOut, Shield } from 'lucide-react';
import { useAppSelector, useAppDispatch } from '@/lib/store/hooks';
import { logout } from '@/lib/store/authSlice';
import Spinner from '@/components/Spinner';

const navItems = [
  { href: '/executive', label: 'Dashboard', icon: LayoutDashboard, exact: true },
  { href: '/executive/projects', label: 'Projects', icon: Briefcase },
  { href: '/executive/reports', label: 'Reports', icon: BarChart3 },
  { href: '/executive/team', label: 'Team', icon: Users },
];
    ...
    <div className="min-h-screen bg-editorial-cream text-stone-900 font-sans">
      ...
      <aside className="hidden lg:flex flex-col h-screen w-64 fixed left-0 top-0 bg-editorial-cream border-r border-stone-200 py-6 px-4 z-40">
        <div className="mb-8 px-2">
          ...
          <p className="text-[9px] font-bold text-stone-500 uppercase tracking-widest leading-none">Executive Terminal</p>
          <div className="w-full h-[1px] bg-stone-200 mt-4"></div>
        </div>

        <div className="flex-grow">
          <p className="text-[9px] font-bold text-stone-400 uppercase tracking-widest px-3 mb-1">Command Center</p>
          {navItems.map((item) => {
        ...
        <div className="mt-auto pt-6 border-t border-stone-200/60 space-y-3">
          {user?.role === 'admin' && (
            <Link
              href="/admin/users"
              className="w-full flex items-center gap-3 px-3 py-2 text-stone-500 hover:text-editorial-wine transition-colors rounded-md text-xs font-medium"
            >
